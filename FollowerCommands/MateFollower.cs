...
public class MateFollower : CustomFollowerCommand
{
    public override string InternalName => "MATE_COMMAND_FOLLOWER";
    ...
    public override bool IsAvailable(Follower follower)
    {
        return TaskUtils.GetAvailableStructureOfType<MatingTent>() != null && FollowerBrain
            .AllAvailableFollowerBrains().Any(x =>
                x != follower.Brain && !FollowerManager.FollowerLocked(x.Info.ID, true) &&
